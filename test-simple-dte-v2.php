...
<?php
/**
 * Script de Pruebas Simple DTE - Versión 2 (Flujo Correcto)
...
echo "=== SIMPLE DTE - SCRIPT DE PRUEBAS V2 (FLUJO CORRECTO) ===\n\n";
...
// Usar siguiente folio disponible (1890 en vez de 1889)
$folio_prueba = 1890;
...
// Paso 3: Generar DTE via Simple API
...
if ($http_code != 200) {
    die("❌ Error HTTP $http_code: $response\n");
}

$dte_xml = $response;
file_put_contents('/tmp/boleta_v2.xml', $dte_xml);
echo "✓ DTE generado exitosamente (" . strlen($dte_xml) . " bytes)\n\n";

// Paso 4: Generar Sobre de Envío (EnvioBoleta firmado)
echo "📦 PASO 4: Generando sobre de envío (EnvioBoleta) vía Simple API...\n";
echo "---------------------------------------------------\n";
...
if ($http_code2 != 200) {
    die("❌ Error HTTP $http_code2: $response2\n");
}

$sobre_xml = $response2;
file_put_contents('/tmp/sobre_firmado_v2.xml', $sobre_xml);
echo "✓ Sobre generado y firmado exitosamente (" . strlen($sobre_xml) . " bytes)\n\n";
...
echo "  - /tmp/boleta_v2.xml\n";
echo "  - /tmp/sobre_firmado_v2.xml\n";
if (isset($track_id)) {
    echo "  - /tmp/track_id_v2.txt\n";
}
echo "\n";
