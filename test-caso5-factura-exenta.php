...
<?php
/**
 * CASO-5: Factura Exenta Electrónica (DTE 34)
 * Documento tributario para ventas exentas de IVA
 */

error_reporting(E_ALL);
ini_set('display_errors', 1);

echo "=== CASO-5: FACTURA EXENTA ELECTRÓNICA ===\n\n";

// Configuración
define('API_KEY', '9794-N370-6392-6913-8052');
define('CERT_PATH', __DIR__ . '/16694181-4.pfx');
define('CERT_PASSWORD', '5605');
define('CAF_PATH_34', __DIR__ . '/FoliosSII782742253420251191419.xml'); // CAF para tipo 34
...
// Verificar CAF
if (!file_exists(CAF_PATH_34)) {
    echo "❌ ATENCIÓN: No se encontró el archivo CAF para Factura Exenta (Tipo 34)\n\n";
    echo "Para generar una Factura Exenta, necesitas:\n";
    echo "1. Ingresar a https://mipyme.sii.cl\n";
    echo "2. Ir a 'Folios' → 'Generar Folios'\n";
    echo "3. Seleccionar DTE tipo 34 (Factura Exenta Electrónica)\n";
    echo "4. Solicitar folios (ejemplo: 100 folios)\n";
    echo "5. Descargar el archivo CAF\n";
    echo "6. Guardar como: " . CAF_PATH_34 . "\n\n";
    exit(1);
}

$caf_xml = file_get_contents(CAF_PATH_34);
$caf = simplexml_load_string($caf_xml);
$folio = (int) ((string) $caf->CAF->DA->RNG->D);

echo "📋 CAF Factura Exenta cargado\n";
echo "  Folio a usar: {$folio}\n\n";

echo "📄 Generando Factura Exenta...\n";
echo "  Cliente: Fundación Sin Fines de Lucro\n";
echo "  RUT: 88888888-8\n\n";

// Productos/servicios exentos de IVA
$items_data = [
    ['nombre' => 'Libro educativo', 'cantidad' => 5, 'precio_unitario' => 15000],
    ['nombre' => 'Material didáctico', 'cantidad' => 10, 'precio_unitario' => 8000],
    ['nombre' => 'Servicio de capacitación', 'cantidad' => 2, 'precio_unitario' => 50000],
];

$monto_exento = 0;
foreach ($items_data as $item) {
    $monto_exento += $item['cantidad'] * $item['precio_unitario'];
}

$monto_total = $monto_exento;

echo "  Monto Exento: $" . number_format($monto_exento, 0, ',', '.') . "\n";
...
foreach ($items_data as $item) {
    ...
    $detalles[] = [
        'IndicadorExento' => 1,  // 1 = Item exento de IVA
        'Nombre' => $item['nombre'],
...
$documento = [
    'Documento' => [
        'Encabezado' => [
            ...
            'Receptor' => [
                'Rut' => '88888888-8',
                'RazonSocial' => 'Fundación Sin Fines de Lucro',
                'Giro' => 'Educación y cultura',
...
$body .= '--' . $boundary . $eol;
$body .= 'Content-Disposition: form-data; name="files2"; filename="' . basename(CAF_PATH_34) . '"' . $eol;
...
echo "✓ Factura Exenta generada\n\n";
file_put_contents('/tmp/factura_exenta_caso5.xml', $dte_xml);
...
$body2 .= '--' . $boundary2 . $eol;
$body2 .= 'Content-Disposition: form-data; name="files"; filename="factura_exenta.xml"' . $eol;
...
echo "\n=== CASO-5 COMPLETADO ===\n";
echo "Archivo generado: /tmp/factura_exenta_caso5.xml\n\n";
